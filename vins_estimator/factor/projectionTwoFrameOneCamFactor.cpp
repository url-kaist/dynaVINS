...
bool ProjectionTwoFrameOneCamFactor::Evaluate(double const *const *parameters, double *residuals, double **jacobians) const
{
...
#endif
    prevResidual = sqrt_info * residual;
    ...
    if (jacobians)
    {
        ...
        if (jacobians[3])
        {
            ...
            jacobian_feature = sel * reduce * ric.transpose() * Rj.transpose() * Ri * ric * pts_i_td * -1.0 / (inv_dep_i * inv_dep_i);
        }
        if (jacobians[4])
        {
            Eigen::Map<Eigen::Vector2d> jacobian_td(jacobians[4]);
            jacobian_td = sel * reduce * ric.transpose() * Rj.transpose() * Ri * ric * velocity_i / inv_dep_i * -1.0  +
                          sqrt_info * velocity_j.head(2);
        }
        if (jacobians[5])
        {
          Eigen::Map<Eigen::Vector2d> jacobian_sel(jacobians[5]);
          jacobian_sel = prevResidual;//.cwiseAbs();
        }
    }
    sum_t += tic_toc.toc();
