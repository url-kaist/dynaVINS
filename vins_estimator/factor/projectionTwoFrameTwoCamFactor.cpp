...
bool ProjectionTwoFrameTwoCamFactor::Evaluate(double const *const *parameters, double *residuals, double **jacobians) const
{
    ...
    residual = sel * sqrt_info * residual;
    ...
    if (jacobians)
    {
        ...
        if (jacobians[0])
        {
            ...
            jacobian_pose_i.leftCols<6>() = sel * reduce * jaco_i;
        ...
        if (jacobians[1])
        {
            ...
            jacobian_pose_j.leftCols<6>() = sel * reduce * jaco_j;
        ...
        if (jacobians[2])
        {
            ...
            jacobian_ex_pose.leftCols<6>() = sel * reduce * jaco_ex;
        ...
        if (jacobians[3])
        {
            ...
            jacobian_ex_pose1.leftCols<6>() = sel * reduce * jaco_ex;
...
#if 1
            jacobian_feature = sel * reduce * ric2.transpose() * Rj.transpose() * Ri * ric * pts_i_td * -1.0 / (inv_dep_i * inv_dep_i);
#else
        ...
        if (jacobians[5])
        {
            ...
            jacobian_td = sel * reduce * ric2.transpose() * Rj.transpose() * Ri * ric * velocity_i / inv_dep_i * -1.0  +
                          sqrt_info * velocity_j.head(2);
        }
        if (jacobians[6])
        {
          Eigen::Map<Eigen::Vector2d> jacobian_sel(jacobians[6]);
          jacobian_sel =  sqrt_info * ((pts_camera_j / dep_j).head<2>() - pts_j_td.head<2>());
        }
    }
    sum_t += tic_toc.toc();
