...
public:
  ...
  void getIndexById(const vector<int> absId,vector<int> & index);
  void computeCurrentBRIEFPoint();
  void removeFeature(const vector<int> featureId);
  ...
  bool findConnection(KeyFrame* old_kf,Vector3d& rel_t,Quaterniond& rel_q, double& rel_yaw,
  ...
  void PnPRANSAC(const vector<cv::Point2f> &matched_2d_old_norm,
	...
	cv::Mat image;
	cv::Mat thumbnail;
  vector<cv::Point3f> point_3d_ref;
	vector<cv::Point3f> point_3d; 
	vector<cv::Point2f> point_2d_uv;
	vector<cv::Point2f> point_2d_norm;
  vector<int> point_id;
  vector<double> point_weight;
  vector<int> dynamic_id;
