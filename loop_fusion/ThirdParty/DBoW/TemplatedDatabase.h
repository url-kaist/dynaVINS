...
public:
  ...
  void queryL1NEW(const std::vector<TDescriptor> &features, QueryResults &ret,
    int max_results, int max_id,int cur_id) const;
...
template<class TDescriptor, class F>
void TemplatedDatabase<TDescriptor, F>::queryL1NEW(
    const std::vector<TDescriptor> &features,
  QueryResults &ret, int max_results, int max_id,int cur_id) const
{
  ...
  for(vit = vec.begin(); vit != vec.end(); ++vit)
  {
    ...
    for(rit = row.begin(); rit != row.end(); ++rit)
    {
      ...
      if((int)entry_id < max_id || max_id == -1|| (int)entry_id == cur_id - 1)
      {
        double value = fabs(qvalue - dvalue) - fabs(qvalue) - fabs(dvalue);
