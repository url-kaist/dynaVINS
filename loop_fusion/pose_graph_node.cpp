...
float MAX_HYPODIFF_DIST;
float MAX_HYPODIFF_YAW;
float HYPOPRIOR_GAMMA;
float HYPOPRIOR_ALT_CONV;
    ...
    while (true)
    {
        ...
        if (pose_msg != NULL)
        {
            ...
            if((T - last_t).norm() > SKIP_DIS)
            {
                ...
                vector<int> point_id,dynamic_id;
                vector<double> point_weight;
                ...
                for (unsigned int i = 0; i < point_msg->points.size(); i++)
                {
                    ...
                    double p_id, p_weight;
                    p_2d_normal.x = point_msg->channels[i].values[0];
                    p_2d_normal.y = point_msg->channels[i].values[1];
                    p_2d_uv.x = point_msg->channels[i].values[2];
                    p_2d_uv.y = point_msg->channels[i].values[3];
                    p_id = point_msg->channels[i].values[4];
                    p_weight = point_msg->channels[i].values[5];
                    point_2d_normal.push_back(p_2d_normal);
                    point_2d_uv.push_back(p_2d_uv);
                    point_id.push_back((int)p_id);
                    point_weight.push_back(p_weight);
                }

                KeyFrame* keyframe = new KeyFrame(pose_msg->header.stamp.toSec(), frame_index, T, R, image,
                                   point_3d, point_2d_uv, point_2d_normal, point_id,point_weight, sequence);
                m_process.lock();
                start_flag = 1;
...
int main(int argc, char **argv)
{
    ...
    MIN_SCORE = fsSettings["min_score_static"];
    MIN_OVERLAP = fsSettings["min_overlap"];
    MAX_KEYFRAME = fsSettings["max_keyframe"];
    MAX_HYPODIFF_DIST = fsSettings["hypodiff_dist"];
    MAX_HYPODIFF_YAW = fsSettings["hypodiff_yaw"];
    HYPOPRIOR_GAMMA = fsSettings["hypo_regularization"];
    HYPOPRIOR_ALT_CONV = fsSettings["hypo_alternating_converge"];
    MIN_LOOP_NUM = fsSettings["min_loop"];
    ...
    std::string pkg_path = ros::package::getPath("dynaVINS");
...
//    VINS_RESULT_PATH = VINS_RESULT_PATH + "/vio_loop.csv";
    std::ofstream fout(VINS_RESULT_PATH+ "/vio_loop.csv", std::ios::out);
    fout.close();
